...
export class SkateboardGame {
  constructor({ canvas }) {
    ...
    this.scene.fog = new THREE.Fog(0x8fc4e8, 45, 110);
    ...
    const hemi = new THREE.HemisphereLight(0xdfefff, 0x6a5f4a, 0.95);
    this.scene.add(hemi);
  ...
  // ── 場景:U 型池 + 池畔 + 棕櫚樹 + 看台觀眾(LA 2028 街景) ──
  buildPark() {
    // ★地形(C4 地基):位移網格,建一次
    this.terrain = createTerrainMesh({ segX: 24, segZ: 160 });
    ...
    const deckMat = new THREE.MeshStandardMaterial({ color: 0x9aa0a8, roughness: 1 });
    ...
    const groundMat = new THREE.MeshStandardMaterial({ color: 0xcabb9a, roughness: 1 });
    ...
    // 池身外壁(讓 U 型池看起來是實體)
    const wallMat = new THREE.MeshStandardMaterial({ color: 0x8b9199, roughness: 0.95 });
    for (const side of [-1, 1]) {
    ...
    const palmSpots = [[-16, -12], [-8, -13.5], [4, -12.5], [13, -13], [-13, 12.5], [9, 13]];
    for (const [px, pz] of palmSpots) {
      const tree = new THREE.Group();
      for (let i = 0; i < 5; i++) {
        const seg = new THREE.Mesh(new THREE.CylinderGeometry(0.16 - i * 0.015, 0.19 - i * 0.015, 1.0, 8), trunkMat);
        seg.position.set(Math.sin(i * 0.35) * 0.35, 0.5 + i * 0.95, 0);
        tree.add(seg);
      }
      for (let k = 0; k < 6; k++) {
        const leaf = new THREE.Mesh(new THREE.ConeGeometry(0.34, 2.6, 5), leafMat);
        const a = (k / 6) * Math.PI * 2;
        leaf.position.set(Math.sin(4 * 0.35) * 0.35 + Math.cos(a) * 1.0, 5.15, Math.sin(a) * 1.0);
  ...
  // 滑板選手(★臉部鐵則+裝束寫實:安全帽/T恤/短褲/護膝護肘/板鞋;矩形身體+長腿)
  makeSkater() {
    const g = new THREE.Group();
    g.rotation.order = "YXZ";
    const skin = new THREE.MeshStandardMaterial({ color: 0xf0d3aa, roughness: 0.7, emissive: 0x7a6446, emissiveIntensity: 0.4 });
    const tee = new THREE.MeshStandardMaterial({ color: 0xe8503a, roughness: 0.9 });
    const shorts = new THREE.MeshStandardMaterial({ color: 0x2b3f5c, roughness: 0.95 });
    ...
    const shoe = new THREE.MeshStandardMaterial({ color: 0xf2f2f2, roughness: 0.85 });
    const dark = new THREE.MeshBasicMaterial({ color: 0x23190f });
    const white = new THREE.MeshBasicMaterial({ color: 0xffffff });

    // 滑板(板身+四輪+前後翹頭)
    const board = new THREE.Group();
    const deckMat = new THREE.MeshStandardMaterial({ color: 0x6b4a2c, roughness: 0.8 });
    const deck = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.05, 0.84), deckMat);
    board.add(deck);
    for (const e of [-1, 1]) {
      const kick = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.05, 0.14), deckMat);
      kick.position.set(0, 0.035, e * 0.46);
      kick.rotation.x = -e * 0.42;
      board.add(kick);
    }
    const wheelM = new THREE.MeshStandardMaterial({ color: 0xffd23f, roughness: 0.5 });
    for (const wx of [-0.1, 0.1]) for (const wz of [-0.28, 0.28]) {
      const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.055, 0.05, 10), wheelM);
    ...
    // 腿(pivot 在髖;滑板站姿=雙腳前後開、微蹲)——長腿 v2 比例
    const mkLeg = (z) => {
  ...
  endRun() {
    ...
    const text = stars === 3
      ? `總分 ${this.score}!高度、轉體、抓板全都到位——你就是這座 U 型池今天的主角!🛹`
      : stars === 2
